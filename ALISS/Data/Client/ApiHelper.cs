...
namespace ALISS.Data.Client
{
    public class ApiHelper
    {
        ...
        public async Task<string> DownloadDataAsync<T>(string apiName, FileInfo outputFile, T model)
        {
            log.MethodStart();
            var data = JsonSerializer.Serialize(model);
            HttpContent content = new StringContent(data, Encoding.UTF8, "application/json");
        ...
        public async Task<string> ExportToPdfDataAsync(string apiName, string[] model)
        {
            log.MethodStart();
